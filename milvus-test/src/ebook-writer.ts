...
async function ensureCollection() {
  try {
    ...
    console.error('创建集合时出错:', error instanceof Error ? error.message : String(error));
    throw error;
  }
}
...
async function loadAndProcessEPubStreaming(bookId: number) {
  try {
    ...
    console.error('加载 EPUB 文件时出错:', error instanceof Error ? error.message : String(error));
    throw error;
  }
}
